function AugmentedMatrix_Algorithm_Benchmark(NodeRes_file,LP_file)
...
U_eva           = 1.02 * 400/sqrt(3); % Set voltage amount of evaluation (eva) point of linearization
...
start_TimeStep  = 1;
% end___TimeStep  = 5000;
end___TimeStep  = 365*24*6;

%% Path and directory preparation

addpath(           [cd,'\Subfunctions' ]);  % Add subfunction path
addpath(           [cd,'\Static_Input\']);  % Add static input path
% LP_Path          = [cd,'\LFC_Results\' ];   % Get Load Flow calculation results path
LP_Path          = 'Z:\Gassler\2_Sim\Lastfluss\Ergebnisse LFR Scada\';
SincalModel.Path = [cd,'\Sincal_Grids\'];   % Get Sincal Grids path
...
elseif  exist(LP_file,'file')
    load([LP_Path,NodeRes_all_Name])
else
...
try
    ...
    load([SincalModel.Path(1:end-13),'Static_Input\','SincalModel.mat']);
catch
    SincalModel.Name = 'Wessum-Riete_Netz_170726_empty';    % Get sincal model Name
    SincalModel.Info = ...
...
NodeRes_all_back = NodeRes_all;
% load('Z:\Gassler\5_SYNTH_Data\Load_Profiles\DB111_10min_synth.mat');
load(NodeRes_file);
NodeRes_all = insertPseudoValues(NodeRes_all,SincalModel.Info,Load_Profiles,'random');
...
for TimeStep = start_TimeStep:end___TimeStep
    ...
    % phi all three phase
    NodeRes_Column_t(:,2)	= deg2rad(reshape(transp([...
        NodeRes_t(:,3),...
        NodeRes_t(:,8),...
        NodeRes_t(:,13)]),[],1));
    
    % P   all three phase
    NodeRes_Column_t(:,3)   = reshape(transp([...
        NodeRes_t(:,4),...
        NodeRes_t(:,9),...
        NodeRes_t(:,14)]),[],1)*10^6;
    
    % Q   all three phase
    NodeRes_Column_t(:,4)   = reshape(transp([...
        NodeRes_t(:,5),...
        NodeRes_t(:,10),...
        NodeRes_t(:,15)]),[],1)*10^6;
    ...
    z_estimate_____all      = H_AM * x_estimate(:,k);	% Calculate estimated measurement values with the help of the estimated network state
    z_estimate_virtual(:,k) = C_AM * x_estimate(:,k);
    z_estimate____meas(:,k) = z_estimate_____all(                    1 : z_meas____num_all                    );
    z_estimate__pseudo(:,k) = z_estimate_____all(z_meas____num_all + 1 : z_meas____num_all + z_pseudo__num_all);
