...
% Mat2Sin_GetSinInfo - Get information about a Sincal model
% 
...
%       SinInfo    (Result)     - Struct with information about all Nodes 
%                                 and Elements
%
% RB, v2016_02_16
...
% In this loop delete the free spaces of the strings in SinNodes with
% "strtrim"
...
%% Get all element types from the element table in the DB and categorize 
%  them into single, two and three Port
...
for kE = 1:num_of_TwoPortTypes 
    ...
    % SQL command: ['SELECET ', '"Column Names"', ' FROM ' '"Table Name"']
    sql = ['SELECT ', 'Element_ID,ElementName,Terminal_1.Terminal_ID,Terminal_2.Terminal_ID,Node_1.Node_ID,Node_2.Node_ID',...
...
%         ' FROM ', 'QueryTopologyTwoPort', ' WHERE Type = ''' kE_TypeName '''' ];
    ...
    SinInfo.(kE_TypeName) = cell2table(SinTwoPort,'VariableNames',{'Element_ID','Name','Terminal1_ID','Terminal2_ID','Node1_ID','Node2_ID'});
