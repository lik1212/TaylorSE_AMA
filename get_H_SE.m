...
%         end
        delta_eva = x_hat(k_P + size_Y) - x_hat(k_Pos + size_Y);
        H(2*size_Y+k_P,k_Pos)         = x_hat(k_P)               * (cos(delta_eva)*G_ij(k_P,k_Pos) + sin(delta_eva)*B_ij(k_P,k_Pos));
        H(2*size_Y+k_P,size_Y+k_Pos)  = x_hat(k_P) * x_hat(k_Pos)* (sin(delta_eva)*G_ij(k_P,k_Pos) - cos(delta_eva)*B_ij(k_P,k_Pos));
    end
end
...
%         end
        delta_eva = x_hat(k_P + size_Y) - x_hat(k_Pos + size_Y);
        H(3*size_Y+k_P,k_Pos)          	= x_hat(k_P)               * ( sin(delta_eva)*G_ij(k_P,k_Pos) - cos(delta_eva)*B_ij(k_P,k_Pos));
        H(3*size_Y+k_P,size_Y+k_Pos)    = x_hat(k_P) * x_hat(k_Pos)* (-cos(delta_eva)*G_ij(k_P,k_Pos) - sin(delta_eva)*B_ij(k_P,k_Pos));
    end
end
